...
Running the above code could potentially lead to a red error message regarding dependencies not being available to for the package. The most likely packages listed are the `gdsfmt`, `SNPRelate`, and `SeqArray` packages. To install these packages from Bioconductor, try the following commands:
...
> **what do a and b represent in the equation? would we wanna clarify for the vignette** 
...
**Note:** Computation time increases with the number of replications, so for the purposes of this example we choose a small number for `nreps`. **In practice, we recommend using a much larger number number of replications (the STEAM default is `nreps = 10,000`).** ***MENTION APPROX. TIMING HERE??***
