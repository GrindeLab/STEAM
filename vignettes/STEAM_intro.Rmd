...
Running the above code could potentially lead to an error message regarding dependencies not being available to for the package. The most likely packages listed are the `gdsfmt`, `SNPRelate`, and `SeqArray` packages. To install these packages from Bioconductor, try the following commands:
...
Running the code above will download STEAM as well as automatically install the following dependent packages: `BioGenerics`, `BiocVersion`, `Biostrings`, `bootstrap`, `data.table`, `GenomeInfoDb`, `GenomeInfoDbData`, `GenomicRanges`, `IRanges`, `Rcpp`, `S4Vectors`, `UCSC.utils`, `XVector`, and `zlibbioc.`

\

Depending on how many of the dependent packages are already downloaded and installed on your machine, the download process could take up to 10 minutes to complete.

If asked to update any packages to the more recent versions available, we recommend that you do so. This can be done by typing "1" into the console when prompted during the download process.
...
* `alpha`: the level for family-wise error rate control (default value is 0.05)
* `method`: the method used to simulate the test statistics, "cpp" for cpp (using Rcpp, approximately 80% faster than "r") or "r" for R (default is "cpp")
...
By default, `get_thresh_simstat` uses the package Rcpp developed by [RcppCore](https://www.rcpp.org/) to speed up the simulation process by approximately 80% by using C++ code rather than R code. If you would like to use the slower R code version instead, include the argument `method = "r"`:
