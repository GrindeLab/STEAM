...
To install the STEAM package from GitHub, run the following commands in `R`:
...
Running the above code could lead to an error message regarding dependencies not being available to for the package. The most likely packages listed are the `gdsfmt`, `SNPRelate`, and `SeqArray` packages. To install these packages from Bioconductor, try the following commands:
...
If asked to update any packages to the more recent versions available, we recommend that you do so. This can be done by typing "1" or "a" into the console when prompted during the download process, depending on which message you are prompted with.

**NOTE:** If you are using the online Posit Workbench version of RStudio in a browser on your machine, the download process will take significantly longer than if using the desktop version.
...
Store this local ancestry correlation in a data frame with three columns, as in the following examples (available to download from the [STEAM GitHub repository](https://github.com/GrindeLab/STEAM/tree/main/data)):
...
One option (if you are studying an admixed population with `K = 3` ancestral populations) is to use the functions `get_corr_chr` and `combine_corr_chr` contained in this package:
...
More information about obtaining a `map` data frame can be found in **Step 1** of Section 3. The example data frame `example_map` can be downloaded from the [STEAM GitHub repository](https://github.com/GrindeLab/STEAM/tree/main/data).
...
The following code with the inclusion of `alpha = 0.01` changes the desired family-wise error rate to from the default of 0.05 to a new rate of 0.01. `alpha` can be any value within the range [0,1]:
...
* `alpha`: the level for family-wise error rate control (default value is 0.05)
* `method`: the method used to simulate the test statistics, "cpp" for cpp (using Rcpp, significantly faster than "r") or "r" for R (default is "cpp")

\

Our `K = 2` ancestral population example data, `example_corr` (used in the `get_g` function) has a `g` value of 6. Our `K = 3` ancestral population example data, `example_corr_K3` (used in the `get_g` function) has a `g` value of 10.

\

As `get_thresh_simstat` is running, a progress bar will appear in the console to show how far along the program is in the simulation process. Depending on the size of the data and the value for `nreps`, the function will take a few minutes to run and the progress can be tracked visually.
